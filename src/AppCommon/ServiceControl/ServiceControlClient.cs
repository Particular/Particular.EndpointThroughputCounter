namespace Particular.EndpointThroughputCounter.ServiceControl
{
    ...
    class ServiceControlClient
    {
        ...
        public ServiceControlClient(string paramName, string instanceType, string rootUrl, Func<HttpClient> httpFactory,
            bool tryUnsupportedVersion = false)
        {
            if (string.IsNullOrWhiteSpace(rootUrl))
            {
                throw new HaltException(HaltReason.InvalidConfig,
                    $"The {paramName} option specifying the {instanceType} URL was not provided.");
            }
        ...
        public async Task<TJsonType> GetData<TJsonType>(string pathAndQuery, int tryCount,
            CancellationToken cancellationToken = default)
        {
            ...
            throw new InvalidOperationException(
                "Retry loop ended without returning or throwing. This should not happen.");
        }
        ...
        public async Task CheckEndpoint(Func<string, bool> contentTest, CancellationToken cancellationToken = default)
        {
            ...
            catch (HttpRequestException hx)
            {
                throw new HaltException(HaltReason.InvalidEnvironment,
                    $"The server at {rootUrl} did not respond. The exception message was: {hx.Message}");
            }

            if (!res.IsSuccessStatusCode)
            {
                var b = new StringBuilder(
                        $"The server at {rootUrl} returned a non-successful status code: {(int)res.StatusCode} {res.StatusCode}")
                    .AppendLine()
            ...
            {
                throw new HaltException(HaltReason.InvalidConfig,
                    $"The server at {rootUrl} specified by parameter {paramName} does not appear to be a ServiceControl instance. Are you sure you have the right URL?");
            }
            ...
            {
                var unsupportedVersionMessage =
                    $"The {instanceType} instance at {rootUrl} is running version {Version}. The minimum supported version is {MinServiceControlVersion.ToString(3)}.";
            ...
            {
                throw new HaltException(HaltReason.InvalidConfig,
                    $"The server at {rootUrl} specified by parameter {paramName} does not appear to be a {instanceType} instance. Are you sure you have the right URL?");
            }
        }
    }
}